...
#[cfg(test)]
mod mascot_window_tests {
    use super::MascotWindowState;

    #[test]
    fn starts_with_default_size_and_no_initialized_window() {
        let state = MascotWindowState::new();

        assert_eq!(state.size().width, 420);
        assert_eq!(state.size().height, 720);
        assert!(!state.is_initialized());
        assert!(!state.is_visible());
    }

    #[test]
    fn resize_updates_cached_size_even_without_window_handle() {
        let state = MascotWindowState::new();

        state.resize(512, 384).expect("resize should succeed");

        assert_eq!(state.size().width, 512);
        assert_eq!(state.size().height, 384);
    }

    #[test]
    fn show_and_hide_update_visible_state_without_a_window_handle() {
        let state = MascotWindowState::new();
    ...
    #[test]
    fn resize_clamps_to_mascot_window_limits() {
        let state = MascotWindowState::new();

        state.resize(32, 32).expect("resize should succeed");
        assert_eq!(state.size().width, 180);
        assert_eq!(state.size().height, 180);

        state.resize(4096, 4096).expect("resize should succeed");
        assert_eq!(state.size().width, 1200);
        assert_eq!(state.size().height, 1200);
    }
}
